...
export const createSearchIndex = (persistentData: PersistentData, data: CreateSearchIndexData) => {
  // TODO: Find a way to delay the conversion until needed i.e. if index is not present
  const { result: initList, nodeBlockMap: nbMap } = convertDataToIndexable(persistentData)
